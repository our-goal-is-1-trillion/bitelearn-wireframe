  ...
  const rawSpecs = [
    {
      ...
      items: [
         ...
         { title: "전역 진도율 트래킹 (Progress Tracking)", priority: "P0", desc: "전체 커리큘럼에서 사용자 계정(User ID)에 종속된 챕터별 상태값(Locked, Available, In-progress, Completed)을 DB와 연동/추적합니다. 이 데이터를 기반으로 홈 대시보드의 '목표 달성률', '이어하기 딥링크 뷰', '카테고리 맵 비활성화 처리' 등 모든 진도 관련 UI를 일괄 업데이트(수학적 Count 로직 등)합니다." },
    ...
    },
    {
      ...
      items: [
         ...
         { title: "마이크로러닝 틴더 뷰어 (Word Cards)", priority: "P0", desc: "가벼운 개념/단어 확인 모드 플로우입니다. Tinder 형태의 좌우 스와이프 제스처 인터페이스가 지원되며, 상단에 현재 카드/전체 카드의 비율을 보여주는 게이지 인디케이터를 연동합니다." },
