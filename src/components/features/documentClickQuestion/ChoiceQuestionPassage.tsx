import { useLayoutEffect, useRef, useState } from "react"
import { motion, AnimatePresence } from "framer-motion"
import QuizFooter from "@/components/layout/QuizFooter"
import { Check } from "lucide-react"

type DocumentCardField = {
  label: string
  value: string
}

type DocumentCardData = {
  header: string
  subHeader: string
  sectionTitle: string
  fields: DocumentCardField[]
  footerNotice?: string
}

type ChoiceQuestionPassageProps = {
  passage: string
  flavorText: string
  questionText?: string
  questionNumber?: number
  hidePassage?: boolean
  hideFlavorText?: boolean
  passageMode?: "text" | "story" | "document"
  documentCard?: DocumentCardData
  choiceMode?: "multiple" | "ox" | "document_select"
  selectedValue?: string
  onSelectDocumentField?: (value: string) => void
  isChecking?: boolean
  correctIndex?: number
  onSolve: () => void
  hideSolveButton?: boolean
}

export default function ChoiceQuestionPassage({
  passage,
  flavorText,
  questionText,
  questionNumber,
  hidePassage = false,
  hideFlavorText = false,
  passageMode = "text",
  documentCard,
  choiceMode = "multiple",
  selectedValue = "",
  onSelectDocumentField,
  isChecking = false,
  correctIndex,
  onSolve,
  hideSolveButton = false,
}: ChoiceQuestionPassageProps) {
  const isDocumentMode = passageMode === "document" && !!documentCard
  const valueRefs = useRef<Record<number, HTMLSpanElement | null>>({})
  const [isWrappedMap, setIsWrappedMap] = useState<Record<number, boolean>>({})

  useLayoutEffect(() => {
    const measure = () => {
      const next: Record<number, boolean> = {}
      Object.entries(valueRefs.current).forEach(([key, el]) => {
        if (!el) return
        const style = window.getComputedStyle(el)
        const lineHeight = Number.parseFloat(style.lineHeight || "0")
        next[Number(key)] = lineHeight > 0 ? el.scrollHeight > lineHeight + 1 : false
      })
      setIsWrappedMap(next)
    }

    measure()
    window.addEventListener("resize", measure)
    return () => window.removeEventListener("resize", measure)
  }, [documentCard, selectedValue, isChecking, choiceMode, questionText, hidePassage, hideFlavorText])

  return (
    <>
      <section className="flex-1 overflow-y-auto px-6 pb-10" data-mode={passageMode}>
        {questionText && (
          <h2 className="mb-6 text-lg font-bold text-slate-900 tracking-tight leading-tight">
            <span className="text-slate-400 mr-2">{questionNumber ? `Q${questionNumber}.` : ""}</span>
            {questionText}
          </h2>
        )}

        {!hidePassage && (
          <div className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm mb-6">
            <p className="text-sm leading-relaxed text-slate-600 font-medium">{passage}</p>
          </div>
        )}

        {!hideFlavorText && (
          <div className="mb-6 flex items-start gap-2">
            <div className="h-1.5 w-1.5 rounded-full bg-slate-900 mt-2" />
            <p className="text-sm font-bold text-slate-900 leading-relaxed">{flavorText}</p>
          </div>
        )}

        {isDocumentMode && (
          <div className="relative mt-2 rounded-[32px] border-2 border-slate-100 bg-slate-50 p-6 overflow-hidden shadow-inner">
            
            {/* ─── Scanning Animation Layer ─── */}
            <AnimatePresence>
              {isChecking && (
                <motion.div 
                  className="absolute inset-0 z-20 pointer-events-none"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                >
                  {/* Scanner Bar */}
                  <motion.div 
                    className="absolute inset-x-0 h-1 bg-slate-900/40 shadow-[0_0_15px_rgba(15,23,42,0.3)] z-30"
                    initial={{ top: "0%" }}
                    animate={{ top: ["0%", "100%", "0%"] }}
                    transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                  />
                  {/* Scanning Overlay */}
                  <motion.div 
                    className="absolute inset-0 bg-slate-900/5 backdrop-blur-[1px]"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  />
                </motion.div>
              )}
            </AnimatePresence>

            <div className="mb-5 border-b border-slate-200 pb-4">
              <p className="text-sm font-bold text-slate-900 uppercase tracking-widest">{documentCard.header}</p>
              <p className="text-xs font-medium text-slate-400 mt-1">{documentCard.subHeader}</p>
            </div>

            <p className="mb-4 text-xs font-bold text-slate-400 uppercase tracking-tighter">{documentCard.sectionTitle}</p>

            <div className="space-y-3">
              {documentCard.fields.map((field, index) => {
                const isSelected = selectedValue === String(index)
                const isAnswer = index === correctIndex

                // Base style: cleaner lo-fi
                let containerClass = "w-full relative group transition-all"
                let fieldClass = "w-full rounded-2xl border-2 px-4 py-3.5 text-left transition-all duration-200 "

                if (choiceMode === "document_select") {
                  if (isChecking) {
                    // After scanning (result view) - keeping it simple for lo-fi
                    if (isAnswer) {
                      fieldClass += "border-slate-900 bg-slate-900 text-white shadow-lg"
                    } else if (isSelected) {
                      fieldClass += "border-slate-200 bg-white text-slate-300 opacity-50"
                    } else {
                      fieldClass += "border-slate-100 bg-transparent text-slate-300 opacity-30"
                    }
                  } else if (isSelected) {
                    // Focused state
                    fieldClass += "border-slate-900 bg-white text-slate-900 shadow-xl shadow-slate-200 translate-y-[-2px] ring-4 ring-slate-100"
                  } else {
                    // Idle state
                    fieldClass += "border-white bg-white text-slate-900 hover:border-slate-200 shadow-sm"
                  }
                }

                return (
                  <div key={`${field.label}-${index}`} className={containerClass}>
                    <button
                      type="button"
                      className={fieldClass}
                      onClick={() => onSelectDocumentField?.(String(index))}
                      disabled={isChecking || choiceMode !== "document_select"}
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className={isWrappedMap[index] ? "flex flex-col items-start gap-1" : "flex items-start gap-3"}>
                          <span className="shrink-0 text-xs font-bold uppercase tracking-wider opacity-60">{field.label}</span>
                          <span
                            ref={(el) => {
                              valueRefs.current[index] = el
                            }}
                            className="min-w-0 basis-0 flex-1 text-sm font-bold leading-snug break-words"
                          >
                            {field.value}
                          </span>
                        </div>
                        
                        {/* Selected Indicator */}
                        {isSelected && !isChecking && (
                          <motion.div 
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                            className="shrink-0 h-6 w-6 rounded-full bg-slate-900 text-white flex items-center justify-center"
                          >
                            <Check size={14} strokeWidth={3} />
                          </motion.div>
                        )}
                      </div>
                    </button>
                  </div>
                )
              })}
            </div>

            {documentCard.footerNotice ? (
              <p className="mt-5 text-[11px] font-bold text-slate-300 text-center uppercase tracking-widest">
                {documentCard.footerNotice}
              </p>
            ) : null}
          </div>
        )}
      </section>

      {!hideSolveButton && (
        <div className="p-6 bg-white border-t border-slate-50">
          <Button 
            className="w-full h-14 rounded-2xl bg-slate-900 text-white font-bold text-lg shadow-xl shadow-slate-200 active:scale-95 transition-all"
            onClick={onSolve}
          >
            서류 분석 시작하기
          </Button>
        </div>
      )}
    </>
  )
}
